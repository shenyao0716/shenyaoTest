...
public class UserDao {
	...
	public JSONObject uservalid(String n,String p){
		conn=DBConn.getCon();
		try {
			prst=conn.prepareStatement("select * from Admin where username=? and pwd=?");
			prst.setString(1, n);
			prst.setString(2, p);
			...
			while(rs.next()){
				...
				o.put("userpwd", rs.getString("pwd"));
	...
	public List<user> getUserInfoName(int id){
		conn=DBConn.getCon();
		List<user> list=new ArrayList<user>();
		try {
			prst=conn.prepareStatement("select * from Admin where userid=?");
			prst.setInt(1, id);
			rs=prst.executeQuery();
			while(rs.next())
			{
	...
	public int updateUserInfo(JSONObject uinfo){
		...
		try {
			prst=conn.prepareStatement("update Users set username=?,userpwd=?,memo=?,name=?,phone=?,Email=? where userid=?");
			...
			prst.setString(6, uinfo.getString("demail"));
			prst.setString(7, uinfo.getString("did"));
